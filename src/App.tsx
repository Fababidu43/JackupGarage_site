  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1">
        ...
        <div className="section py-3 sm:py-4 lg:py-5 slide-in-left diagonal-cut-top-backslash diagonal-cut-bottom-slash" style={{ background: 'linear-gradient(135deg, #0A0A0A 0%, #1A1A1A 100%)' }}>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-8 items-center">
              <div>
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg flex items-center justify-center mb-2 sm:mb-3 pulse-subtle" style={{ background: '#FF6B35', color: 'white' }}>
                  <Droplets className="w-4 h-4 sm:w-5 sm:h-5" />
                </div>
                
                <h3 className="text-base xs:text-lg sm:text-xl lg:text-2xl font-black mb-1 sm:mb-2 tracking-tight leading-tight uppercase text-white font-futuristic hover-glow-text">
                  Entretiens
                </h3>
                
                <p className="text-xs sm:text-sm leading-relaxed mb-2 sm:mb-3 font-light text-white/80 font-tech">
                  Vidanges, filtres, freins. Interventions propres et rapides, à domicile.
                </p>
                
                <button 
                  onClick={() => document.getElementById('contact')?.scrollIntoView({ behavior: 'smooth' })}
                  className="inline-flex items-center font-semibold text-xs sm:text-sm group uppercase tracking-wide text-orange-400 hover:text-orange-300 font-tech transition-colors hover-lift underline-animate"
                >
                  Demander un devis
                  <ArrowRight className="ml-1 w-3 h-3 sm:w-4 sm:h-4 transition-transform group-hover:translate-x-1" />
                </button>
              </div>
        ...
        {/* Service 2 - Embrayage */}
        <div className="section py-3 sm:py-4 lg:py-5 slide-in-right diagonal-cut-top-slash diagonal-cut-bottom-backslash" style={{ background: 'linear-gradient(135deg, #F8F9FA 0%, #FFFFFF 100%)' }}>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-8 items-center">
              <div className="lg:order-2">
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg flex items-center justify-center text-white mb-2 sm:mb-3 pulse-subtle" style={{ background: '#FF6B35' }}>
                  <Zap className="w-4 h-4 sm:w-5 sm:h-5" />
                </div>
                
                <h3 className="text-base xs:text-lg sm:text-xl lg:text-2xl font-black mb-1 sm:mb-2 tracking-tight leading-tight uppercase text-gray-900 font-futuristic hover-glow-text">
                  Embrayage & Volant Moteur
                </h3>
                
                <p className="text-xs sm:text-sm leading-relaxed mb-2 sm:mb-3 font-light text-gray-700 font-tech">
                  Remplacement complet avec essais. Déplacements étendus sur demande.
                </p>
                
                <button 
                  onClick={() => document.getElementById('contact')?.scrollIntoView({ behavior: 'smooth' })}
                  className="inline-flex items-center font-semibold text-xs sm:text-sm group uppercase tracking-wide hover:opacity-80 font-tech transition-colors hover-lift underline-animate"
                  style={{ color: '#FF6B35' }}
                >
                  Demander un devis
                  <ArrowRight className="ml-1 w-3 h-3 sm:w-4 sm:h-4 transition-transform group-hover:translate-x-1" />
